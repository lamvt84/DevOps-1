...
```sql
CREATE PROCEDURE [dbo].[usp_Dba_WaitStats] @ServerName sysname = @@SERVERNAME
AS
BEGIN
...
Table WaitStats will store all information at the time [dbo].[usp_Dba_WaitStats] is executed

Execute SP [dbo].[usp_Dba_WaitStats] each 60s to get the stats change

```sql
EXEC [dbo].[usp_Dba_WaitStats] @ServerName = @@SERVERNAME
```
