...
IF EXISTS (SELECT 1/0 FROM msdb.dbo.sysjobs WHERE name = 'Job_SlowQueryLogPush')
	EXEC msdb.dbo.sp_delete_job @job_name = N'Job_SlowQueryLogPush', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Job_SlowQueryLogPush', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Insert table SlowQueryLog', 
		...
		@command=N'EXEC dbo.pr_SlowQueryLog_Collect', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Clear ring buffer', 
		...
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'
		IF EXISTS (SELECT 1 FROM sys.dm_xe_sessions WHERE [name] = ''KV_SlowQueryLog'')
			ALTER EVENT SESSION KV_SlowQueryLog ON SERVER STATE = STOP;
		IF NOT EXISTS (SELECT 1 FROM sys.dm_xe_sessions WHERE [name] = ''KV_SlowQueryLog'')
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'10m BufferToTablePush', 
		...
		@schedule_uid=N'3241e545-3d38-4907-8333-30a60e7e2a6f'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
