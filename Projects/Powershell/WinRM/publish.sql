...
PRINT HOST_NAME()
GO
PRINT 'Sync wait stats collector'
GO
USE [master]
GO
...
END
GO
ALTER LOGIN [metricbeat] WITH PASSWORD = N'metricbeat'
GO
GRANT VIEW SERVER STATE TO [metricbeat]
GO
USE [EventMonitoring]
GO
...
GO
IF OBJECT_ID('dbo.sp_WaitStats_Report') IS NULL
    EXEC ('CREATE PROCEDURE dbo.sp_WaitStats_Report AS RETURN 0;');
GO
ALTER PROCEDURE [dbo].[sp_WaitStats_Report] @ServerName sysname = NULL
AS
BEGIN
	SET @ServerName = ISNULL(@ServerName, HOST_NAME())
...
(
    ...
    WHERE name = 'metricbeat'
)
BEGIN
    CREATE USER [metricbeat] FOR LOGIN [metricbeat]
END
GO
GRANT EXEC ON dbo.sp_WaitStats_Report TO [metricbeat]
GO
USE [EventMonitoring]
GO
TRUNCATE TABLE dbo.WaitCategories
...
('MEMORY_ALLOCATION_EXT', 'Memory', 1),
...
('PARALLEL_REDO_TRAN_TURN', 'Replication', 0),
...
('PREEMPTIVE_XE_GETTARGETSTATE', 'Preemptive', 1),
...
('SLEEP_MASTERDBREADY', 'Idle', 1),
('SLEEP_MASTERMDREADY', 'Idle', 1),
('SLEEP_MASTERUPGRADED', 'Idle', 1),
...
('UCS_SESSION_REGISTRATION', 'Other', 0),
...
BEGIN
    DECLARE @databaseName AS NVARCHAR(2048),
            @eventMessage AS NVARCHAR(2048)
    SET @databaseName = REPLACE(REPLACE(DB_NAME(), N'\', N'\\'), N'"', N'\"')
    SET @eventMessage
        = N'DBA PS Automation: { "deployment": { "description": "Deployed Wait Stats Collector to ' + @databaseName + N'", "database": "'
          + @databaseName + N'" }}'
